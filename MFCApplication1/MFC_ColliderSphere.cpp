...
Engine::CMFC_ColliderSphere::CMFC_ColliderSphere(LPDIRECT3DDEVICE9 pGraphicDev)
...
Engine::CMFC_ColliderSphere::CMFC_ColliderSphere(const CMFC_ColliderSphere& rhs)
	: CComponent(rhs)	
	//, m_vMin(rhs.m_vMin)
	//, m_vMax(rhs.m_vMax)
...
Engine::CMFC_ColliderSphere::~CMFC_ColliderSphere(void)
{

}

CMFC_ColliderSphere* Engine::CMFC_ColliderSphere::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _vec3* pPos, const _ulong& dwNumVtx, const _ulong& dwVtxSize)
{
	CMFC_ColliderSphere*	pInstance = new CMFC_ColliderSphere(pGraphicDev);

	if (FAILED(pInstance->Ready_MFC_ColliderSphere(pPos, dwNumVtx, dwVtxSize)))
		Safe_Release(pInstance);

	return pInstance;
}

CComponent* Engine::CMFC_ColliderSphere::Clone(void)
{
	return new CMFC_ColliderSphere(*this);
}

void Engine::CMFC_ColliderSphere::Free(void)
{
#ifdef _DEBUG
	Safe_Release(m_pIB);
	Safe_Release(m_pVB);
...
HRESULT Engine::CMFC_ColliderSphere::Ready_MFC_ColliderSphere(const _vec3* pPos, const _ulong& dwNumVtx, const _ulong& dwVtxSize)
{
	//BYTE* pV;
...
void Engine::CMFC_ColliderSphere::Render_MFC_ColliderSphere(COLLTYPE eType, const _matrix* pMFC_ColliderSphereMatrix)
{
	//m_matCollWorld = *pMFC_ColliderSphereMatrix;

	if (FAILED(D3DXCreateSphere(m_pGraphicDev, m_tSphere.fRadius, 10, 10, &m_pSphere, NULL)))
		return;//E_FAIL;
...
//#ifdef _DEBUG
	...
	//m_pGraphicDev->SetTransform(D3DTS_WORLD, pMFC_ColliderSphereMatrix);
	//m_pGraphicDev->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
	//
//	//m_pGraphicDev->SetTexture(0, m_pTexture[eType]);
	//
