...
CPlayerHelmet::CPlayerHelmet(LPDIRECT3DDEVICE9 pGraphicDev)
	: CGameObject(pGraphicDev)
{

}

CPlayerHelmet::CPlayerHelmet(const CPlayerHelmet& rhs)
	: CGameObject(rhs)
{
...
HRESULT CPlayerHelmet::Ready_Object(void)
{
...
Engine::_int CPlayerHelmet::Update_Object(const _float& fTimeDelta)
{
	_vec3 vDir, vPos;
	...
	if (dynamic_cast<CPlayer*>(m_pPlayer)->Get_bHelmetEquip() && !dynamic_cast<CPlayer*>(m_pPlayer)->Get_ArmorFullEquip())
	{
...
void CPlayerHelmet::Render_Object(void)
{
	if (dynamic_cast<CPlayer*>(m_pPlayer)->Get_bHelmetEquip() && !dynamic_cast<CPlayer*>(m_pPlayer)->Get_ArmorFullEquip())
	{
...
HRESULT CPlayerHelmet::Add_Component(void)
{
	CComponent*			pComponent = nullptr;

	// StaticMesh
	pComponent = m_pMeshCom = dynamic_cast<CDynamicMesh*>(Clone_Proto(L"Proto_Mesh_HideHelmet"));
...
Engine::_bool CPlayerHelmet::Collision_ToObject(const _tchar* pLayerTag, const OBJECT_ID pObjTag)
{
...
CPlayerHelmet* CPlayerHelmet::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CPlayerHelmet*	pInstance = new CPlayerHelmet(pGraphicDev);
